          ...
          {error ? (
            ...
            <div className="flex mb-3">
              {resultMovie?.poster_path ? (
                <img
                  src={`https://image.tmdb.org/t/p/w500/${resultMovie?.poster_path}`}
                  alt={`poster ${resultMovie?.title}`}
                  width={250}
                />
              ) : (
                <img
                  src="https://via.placeholder.com/200x200.png?text=NO+PHOTO"
                  alt="no_photo"
                  width={250}
                />
              )}
              <div className="ml-5">
                <h1 className="font-sans text-3xl mb-5 font-bold">{`${
                  resultMovie?.title
                } (${
                  !resultMovie?.release_date
                    ? 'unknown'
                    : resultMovie?.release_date.split('-')[0]
                }) `}</h1>

                <p className="font-sans mb-5 ">{`User Score: ${
                ...
                <p className="font-sans mb-5">{`${
                  !resultMovie?.overview ? 'unknow' : resultMovie?.overview
                }`}</p>

                <h2 className="font-sans text-2xl mb-5 font-bold">Genres:</h2>

                <p className="font-sans ">{`${genres
                  .map(e => e.name)
                  .join(', ')}`}</p>
              </div>
            </div>
